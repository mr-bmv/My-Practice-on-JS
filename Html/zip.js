...
// мое решение через обычный рекурсивный процесс
const zip = (list1, list2) => {
  if (isEmpty(list1) || isEmpty(list2)){
    return l();
  }
const newList = l(head(list1), head(list2));
return cons(newList, zip(tail(list1), tail(list2)));
}
...
// решение учителя, через итеративный процесс
const zip = (list1, list2) => {
